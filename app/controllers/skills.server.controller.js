...
exports.list = function(req, res) {
	...
	Skill.find(condition).sort('-created').populate('category', 'name').exec(function(err, skills) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
...
exports.getSkillsByCategoryId = function(req, res){
	Skill.find({category : req.param('categoryId'), isActive : true}).sort('-created').exec(function(err, skills) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
